    ...
    <?php if (!$articles->items()->isEmpty()): ?>
        <?php foreach ($articles as $article): ?>
            <article>
                    ...
                    <p>
                        ...
                        <span>
                            ...
                            <?php if ($article->comments_count > 0): ?>
                                <?= $this->Html->link($article->comments_count, ['plugin' => 'Blogger', 'controller' => 'Articles', 'action' => 'view', 'id' => $article->id, '#' => 'comments-thread']); ?>
                            <?php else: ?>
                ...
                <footer>
                    <?= $this->Html->link(__d('blogger', 'Read more') . ' <i class="bi bi-chevron-right"></i>', ['plugin' => 'Blogger', 'controller' => 'Articles', 'action' => 'view', $article->id], ['escape' => false, 'class' => 'btn btn-primary text-light']) ?>
                </footer>
            </article>
            <hr>
        <?php endforeach; ?>
